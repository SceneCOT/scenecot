...
import cv2
import json
from model.llava.vision_tower import process_images
...
class ScanDataLoader(object):
    def __init__(self, cfg, dataset = '') -> None:
        ...
        self.img_process_args = cfg.data.process_args.img_process_args if hasattr(cfg.data, 'process_args') else {}
        ...
        self.img_processer_type = self.img_process_args.get("img_processer", "openai/clip-vit-large-patch14-336")

        if self.img_processer_type in ['openai/clip-vit-base-patch32']:
            self.img_transform = CLIPProcessor.from_pretrained(cfg.data.process_args.img_processer).image_processor
        elif self.img_processer_type in ['navigation_img_processer']:
            pass
        print(f'Scan Data Loader img_processer_type: {self.img_processer_type}')
