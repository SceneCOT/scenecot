...
from .capeval.meteor.meteor import Meteor
from .capeval.rouge.rouge import Rouge
import re
...
class Evaluator():
    def __init__(self):
        ...
        # self.sentence_model = SentenceTransformer('/home/lhxk/.cache/torch/sentence_transformers/sentence-transformers_all-MiniLM-L6-v2')
    ...
    def eval_all(self, answer_all):

        total_count = len(answer_all)
        results_dict = {
            'total_count' : total_count
        }
        ## EM@1
        correct1 = 0
        for answer_pred, answer_gts in answer_all:
        ...
        results_dict['em1'] = correct1 / float(total_count)
        results_dict['em1_strict'] = correct_strict / float(total_count)

        # compute cider, bleu, meteor, rouge
        self.init_corpus()
        for i in range(total_count):
            self.pred_sentence_mp.append([answer_all[i][0]])
            self.gt_sentence_mp.append([answer_all[i][1]])
        ...
        results_dict['cider'] = self.cider_scorer.compute_score(self.gt_sentence_mp, self.pred_sentence_mp)[0]
        results_dict['bleu'] = self.bleu_scorer.compute_score(self.gt_sentence_mp, self.pred_sentence_mp)[0][-1]
        results_dict['meteor'] = self.meteor_scorer.compute_score(self.gt_sentence_mp, self.pred_sentence_mp)[0]
        results_dict['rouge'] = self.rouge_scorer.compute_score(self.gt_sentence_mp, self.pred_sentence_mp)[0]
        return results_dict
    ...
    def eval_instance(self, answer_pred, answer_gts):
        ...
        # compute cider, bleu, meteor, rouge
        self.init_corpus()
        self.pred_sentence_mp = [[answer_pred]]
        self.gt_sentence_mp = [answer_gts]
