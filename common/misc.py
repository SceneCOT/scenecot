...
def default_collate(batch):
    ...
    if isinstance(elem, torch.Tensor):
        try:
            ...
            # custom: in case of inconsistent tensor shape
            return batch
...
def gather_object(object: Any):
    ...
    xla_type = getattr(DistributedType, "XLA", None)
    tpu_type = getattr(DistributedType, "TPU", None)
    if distributed_type in [xla_type, tpu_type]:
